...
class spVIPESmodule(BaseModuleClass):
    ...
    def _get_inference_input(
        ...
        input_dict = {"x": x, "batch_index": batch_index, "species": species, "labels": labels}
        return input_dict
    ...
    def inference(self, x, batch_index, species, labels):
        ...
        label_specie = dict(enumerate(labels))
        library = {i: torch.log(xs.sum(1)).unsqueeze(1) for i, xs in x.items()}  # observed library size

        stats = {}
        for (specie, item), batch in zip(x.items(), batch_index):
            encoder = self.encoders[specie]
            values = encoder(item, specie, batch)
            stats[specie] = values
        private_stats = {}
        shared_stats = {}
